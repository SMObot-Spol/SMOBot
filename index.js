  ...
  if (gqueue.autoplay == false) {
    musicButtons.addComponents(
      new MessageButton()
        ...
        .setStyle("SECONDARY")
        .setDisabled(gqueue.songs.length == 0 && gqueue.played.length == 0)
    );
  } else {
    musicButtons.addComponents(
      new MessageButton()
...
async function getID(chars, eRaid, idBed, freeOnly) {
  ...
  for (const value of valArray) {
    ...
    if (idBeds[idBedIndex].fields.length >= 18) {
      idBeds.push(Object.create(idBed));
      idBeds[idBedIndex].fields = idBeds[idBedIndex].fields.sort(compareRows);
  ...
  if (!isNaN(parseInt(element))) {
    ...
    if (element == "offtank") {
      return allChars
        ...
        .filter(
          (char) =>
            char.tank == true && (char.mdps == true || char.rdps == true)
        )
        .sort(compareObjects)
        .sort(compareClasses);
    } else if (element == "dps") {
...
async function getCharsByIdAndClassAndRole(id, role, classa) {
  if (role == "offtank") {
    return allChars
      ...
      .filter(
        (char) => char.tank == true && (char.mdps == true || char.rdps == true)
      )
      .sort(compareObjects)
      .sort(compareClasses);
  } else if (role == "dps") {
  ...
  await Promise.all(endpoints)
    .then(async function (response) {
      for await (const resp of response) {
        ...
        try {
          script = resp.data.toString();
          subsLeft = script.substring(script.indexOf("ds-statistic',"));
          subs = subsLeft.substring(subsLeft.indexOf('avg_item_lvl":"') + 15);
...
bot.on("ready", async () => {
  ...
  let testrole = testguild.roles.cache.find((role) => role.name == "SBTESTER");
  ...
  bot.guilds.cache.forEach(async (guildt) => {
    let emo = await guildt.emojis.cache.find((emoji) => emoji.name === "skap");
    if (!emo) {
      guildt.emojis
        .create("https://i.ibb.co/xjw072D/Discord-Emote-Maker-2.png", "skap")
        .catch(console.error);
    }
  });
...
bot.on("presenceUpdate", async (oPress, nPres) => {
  const pacc = (await nPres.client.fetchPremiumStickerPacks()).find((e) =>
    e.name.toLowerCase().includes("dog")
  );
  const waveMoji = pacc.stickers.find((e) => e.description.includes("wave"));
  if (nPres.userId === "1179713635321393273" && nPres.status === "online") {
    const can = nPres.guild.channels.cache.get("981934988750618697");
    if (can) {
      can
        .send({
          content: `<@${nPres.userId}>`,
          stickers: [waveMoji],
        })
...
bot.on("interactionCreate", async (interaction) => {
  ...
  let optsQueue,
    optsPlayed = [];
  optsQueue = qsongs.filter((song) => song.title.toLowerCase().includes(focus));
...
bot.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "music" || commandName == "play") {
    ...
    if (options?.getSubcommand() == "queue") {
      let page = options.getInteger("page") ? options.getInteger("page") : 0;
      ...
      if (gqueue.songs.length != 0) {
        ...
        let format = first[0].title.replace(/\(official music video\)/gi, "");
      ...
      partQueue.forEach((song) => {
        idx++;
        let format = song.title.replace(/\(official music video\)/gi, "");
        format = format.replace(/\(official video\)/gi, "");
        format = format.replace(/\/official video\//gi, "");
        format = format.replace(/\|official video\|/gi, "");
        songs += `\`${idx}.\`  ${format}\n`;
    ...
    if (options?.getSubcommand() == "remove") {
      ...
      let rmIndexPlayed = gqueue.played.findIndex((song) => song.url == rm);
    ...
    if (options?.getSubcommand() == "prev") {
      if (gqueue.played.length == 0) {
        musicBed.description = "There are no previous songs in the queue.";
        await interaction.reply({
          embeds: [musicBed],
          ephemeral: true,
        });
    ...
    if (
      ...
      if (song.startsWith("https") && ytdl.yt_validate(song) === "video") {
        url = song;
        ...
        const filter = (i) =>
          i.customId === "mbsongs" && i.user.id === interaction.user.id;
        ...
        gqueue.collector = msg
          ...
          .then(async (i) => {
            ...
            if (gqueue.songs.length == 0) {
              ...
              await gqueue.songs.push(new Song(info.label, i.values[0]));
              await play(gqueue.songs[0]?.url, cguild, cchan);
              ...
              for await (const value of rest) {
                info = urlTable.options.find((option) => option.value == value);
              ...
              if (playnext) {
                ...
                for await (const value of i.values) {
                  ...
                  gqueue.songs.splice(counter, 0, new Song(info.label, value));
                }
              } else {
                for await (const value of i.values) {
                  info = urlTable.options.find(
  ...
  if (commandName === "id") {
    ...
    if (options.getString("class")) {
      ...
      eDesc += ` as ${classes[parseInt(options.getString("class")) - 1][1]}`;
  ...
  if (commandName === "raid") {
    ...
    if (options.getSubcommandGroup(false) == "user") {
      ...
      if (options.getSubcommand() == "add") {
        ...
        allRaids.forEach((raid) => {
          if (raid.raidid == roleId) {
            if (raid.info.characters.find((cr) => cr.userId == uId)) {
      ...
      if (options.getSubcommand() == "remove") {
        ...
        let deleted = allRaids[rIndex].info.characters.splice(cIndex, 1);
      ...
      if (options.getSubcommand() == "promote") {
        ...
        let room = interaction.guild.channels.cache.get(`${raidObject.roomId}`);
        room.setName(newName);

        var qs = `UPDATE raids SET info = '${JSON.stringify(
          raidObject
        )}' WHERE raidid = '${raidObject.roleId}' RETURNING raidid,info `;
      ...
      if (options.getSubcommand() == "create") {
        if (options.getString("instance").toLowerCase() == "ds") {
          raidName = "Dragon Soul";
          raidShort = "DS";
        } else if (options.getString("instance").toLowerCase() == "fl") {
          raidName = "Firelands";
          raidShort = "FL";
        } else if (options.getString("instance").toLowerCase() == "bot") {
          raidName = "Bastion of Twilight";
          raidShort = "BOT";
        } else if (options.getString("instance").toLowerCase() == "bwd") {
          raidName = "Blackwing Descent";
          raidShort = "BWD";
        } else if (options.getString("instance").toLowerCase() == "t4w") {
          raidName = "Throne of the Four Winds";
          raidShort = "T4W";
        }
        ...
        let month = options.getString("day").toLowerCase().split("-")[1];
...
bot.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  if (newMember.roles.cache.some((role) => role.name == "SBTESTER")) {
    ...
    if (testerUsers.includes(newMember.user.id)) {
      testerUsers = testerUsers.filter((uId) => uId !== newMember.user.id);
    }
  }
});
...
bot.on("messageCreate", async (sprava) => {
  ...
  if (
    ["!lisakuvzinrokh", "!lisakarcheology", "!lisakdigger"].includes(
      sprava.content
    )
  ) {
    const dissBed = {
        title: "Archeologist tracker",
        color: 7419530,
        timestamp: Date.now(),
        footer: {
          icon_url: "https://i.ibb.co/vs7BpgP/ss.png",
      ...
      if (voicechan) {
        voicechan.members.forEach((member) => {
          if (member != sprava.member) {
            ...
            } else if (sprava.content.toLowerCase().includes("!mutt")) {
              member.voice.setMute(true);
            } else if (sprava.content.toLowerCase().includes("!unmutt")) {
  ...
  if (oldUserChannel === undefined && newUserChannel !== null) {
    ...
    if (
      guildis.roles.cache.find(
        (ro) =>
          ro.name ==
          "(room)-" + newUserChannel.name.toLowerCase().replace(/\s+/g, "-")
      ) === undefined
    ) {
      guildis.roles
        .create({
        ...
        .then(async (ro) => {
          ...
          if (
            nUser.roles.cache.find((ro) => ro.name == roomRole.name) == null
          ) {
            console.log("////////////////");
            ...
            if (
              ...
              await guildis.channels
                ...
                .then(async (vois) => {
                  ...
                  vois.send({ embeds: [joinBed] }).catch(console.error);
                });
            }
          }
        });
  ...
  if (oldUserChannel === undefined && newUserChannel !== undefined) {
    ...
    if (channellog) {
      if (
        ...
        let joinBed = await channelChange(nUser, "has joined the channel.");
    ...
    if (channellog) {
      let roomRole = guildis.roles.cache.find(
        (r) =>
          r.name ==
          "(room)-" + oldUserChannel.name.toLowerCase().replace(/\s+/g, "-")
      );

      if (oUser.roles.cache.find((ro) => ro.name == roomRole.name) != null) {
      ...
      if (
        ...
        let joinBed = await channelChange(oUser, "has left the channel.");
        joinBed.color = 15158332;
        channellog.send({ embeds: [joinBed] }).catch(console.error);

        if (
          guildis.channels.cache.find((c) => c.name == oldUserChannel.name)
            .members.size < 1
        ) {
    ...
    if (oUser.roles.cache.find((ro) => ro.name == oldRoomRole?.name) != null) {
    ...
    if (channellognew) {
      if (
        ...
        let joinBed = await channelChange(oUser, "has joined the channel.");
        joinBed.color = 3066993;
